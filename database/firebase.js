const admin = require("firebase-admin")

const serviceAccount = {
  type: "service_account",
  project_id: "tokentele-169d4",
  private_key_id: "f1756d08d69a6eb8584d81ba97913182896e0cc0",
  private_key: process.env.FIREBASE_PRIVATE_KEY || "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@tokentele-169d4.iam.gserviceaccount.com",
  client_id: "116691566379728419473",
  auth_uri: "https://accounts.google.com/o/oauth2/auth",
  token_uri: "https://oauth2.googleapis.com/token",
  auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
  client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40tokentele-169d4.iam.gserviceaccount.com",
  universe_domain: "googleapis.com",
}

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  })
}

const db = admin.firestore()
db.settings({ ignoreUndefinedProperties: true })

// SESUAIKAN DENGAN API - gunakan telegram_tokens bukan tokens
const tokensCollection = db.collection("telegram_tokens")

module.exports = {
  db,
  tokensCollection,
  
  updateTokenStatus: async function(tokenId, status) {
    try {
      if (!tokenId || typeof tokenId !== 'string' || tokenId.trim() === '') {
        console.error('[Firebase] Invalid tokenId:', tokenId)
        throw new Error('tokenId must be a non-empty string')
      }
      
      const tokenDoc = tokensCollection.doc(tokenId)
      await tokenDoc.update({
        isActive: status, // SESUAIKAN DENGAN API - gunakan isActive bukan status
        lastActiveAt: new Date().toISOString(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      })
      console.log(`[Firebase] Token ${tokenId} status updated to: ${status}`)
    } catch (error) {
      console.error('[Firebase] Error updating token status:', error.message)
      throw error
    }
  },

  getAllTokens: async function() {
    try {
      const snapshot = await tokensCollection.get()
      const tokens = []
      
      snapshot.forEach((doc) => {
        const data = doc.data()
        // SESUAIKAN DENGAN STRUKTUR API
        tokens.push({
          id: doc.id,
          tokenId: data.tokenId || doc.id,
          token: data.token,
          botId: data.botId,
          botUsername: data.botUsername,
          botFirstName: data.botFirstName,
          isActive: data.isActive !== false, // default true
          createdAt: data.createdAt,
          lastActiveAt: data.lastActiveAt,
          updatedAt: data.updatedAt,
          stats: data.stats || { totalCommands: 0, totalUsers: 0 }
        })
      })
      
      return tokens
    } catch (error) {
      console.error('[Firebase] Error fetching tokens:', error.message)
      return []
    }
  }
}
