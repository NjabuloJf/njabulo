...
export async function GET() {
  try {
    const snapshot = await db.collection("developer_notifications").orderBy("createdAt", "desc").limit(50).get()
    ...
    console.error("[Developer Notifications] Error fetching:", error)
