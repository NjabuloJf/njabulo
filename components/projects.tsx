  ...
  >([
    {
      ...
      github: "https://github.com/njabulo-jb",
      icon: Server,
      decorator: Database
    },
    {
      ...
      github: "https://github.com/gtr-xmd",
      icon: Globe,
      decorator: Zap
    },
  ])
        ...
        <div className="max-w-7xl w-full mx-auto relative z-10">
          <ScrollAnimator className="text-center space-y-4 md:space-y-6 mb-16 md:mb-20">
                ...
                <div className="relative w-24 h-24 md:w-28 md:h-28 rounded-full bg-gradient-to-br from-primary via-primary/90 to-accent p-0.5 shadow-2xl">
                  <div className="w-full h-full rounded-full bg-background/95 backdrop-blur-sm flex items-center justify-center overflow-hidden border border-white/10">
                    <div className="relative w-full h-full">
                      <Image src="https://lannytourl.vestia.icu/api/file/6926fa14fdcdd67351cb6d0c.jpg" alt="Projects Logo" fill className="object-cover scale-110" style={{ maskImage: 'radial-gradient(circle, black 60%, transparent 100%)', WebkitMaskImage: 'radial-gradient(circle, black 60%, transparent 100%)' }} />
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent flex items-center justify-center gap-4">
              <Rocket size={40} className="text-primary hidden sm:block" /> Featured Projects <Terminal size={40} className="text-accent hidden sm:block" />
            </h1>
            <p className="text-base sm:text-lg text-foreground/70 max-w-3xl mx-auto leading-relaxed"> Showcase of recent work with cutting-edge technologies and innovative solutions. </p>
          </ScrollAnimator>
                    ...
                    <div className="flex flex-col flex-1 p-6 md:p-8 space-y-4 relative z-10">
                      <div className="space-y-2">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <h3 className="text-xl md:text-2xl font-bold text-foreground group-hover:text-primary transition-colors duration-300 line-clamp-2"> {project.title} </h3>
                        </div>
                        <p className="text-foreground/60 text-sm md:text-base leading-relaxed"> {project.description} </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {project.tags.map((tag, i) => (
                          <span key={i} className="px-3 py-1 bg-primary/15 text-primary text-xs font-medium rounded-full border border-primary/30 hover:border-primary/60 transition-colors duration-300 flex items-center gap-1 group/tag" >
                            <Sparkles size={12} className="group-hover/tag:scale-110 transition-transform" /> {tag}
                          </span>
                        ))}
                      </div>
                      <div className="p-4 rounded-xl bg-secondary/30 border border-border/50 space-y-3 group/language">
                        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                          <BarChart3 size={16} className="text-primary group-hover/language:scale-110 transition-transform" /> Tech Stack
                        </div>
  ...
  {project.tags.slice(0, 4).map((tag, i) => (
    <div key={i} className="flex items-center gap-2 group/bar">
      ...
      <div className="flex-1 h-1.5 bg-border/50 rounded-full overflow-hidden">
        <div className="h-full bg-gradient-to-r from-primary to-accent transition-all duration-500 group-hover/bar:scale-105" style={{ width: `${Math.random() * 50 + 50}%` }} ></div>
      </div>
...
<div className="flex gap-3 pt-2 mt-auto">
  {project.title === "Njabulo Jb" ? (
    ...
    <>
      ...
      <a href="https://github.com/gtr-xmd" 
        className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border border-primary/50 text-primary rounded-xl hover:bg-primary/10 transition-all duration-300 font-semibold text-sm hover:scale-105 group/code"
      >
        <Github size={16} className="group-hover/code:scale-110 transition-transform" />
        Social github
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
    ...
    <div className="space-y-4">
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={languageStats} cx="50%" cy="50%" labelLine={false} label={({ name, percentage }) => `${name} (${percentage}%)`} outerRadius={80} fill="#8884d8" dataKey="percentage" >
              {languageStats.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
              ))}
            </Pie>
